...
__global__ void muladj(float C, float2* y,float2* x,int N1,int N2, int ni)
{
	int tx = blockIdx.x*blockDim.x + threadIdx.x;
	int ty = blockIdx.y*blockDim.y + threadIdx.y;
	int tz = blockIdx.z*blockDim.z + threadIdx.z;
...
__global__ void modifyst(int* st,int* nvals, int Ntheta, int Nrho, int ss)
{
	int tx = blockIdx.x*blockDim.x + threadIdx.x;
	int ty = blockIdx.y*blockDim.y + threadIdx.y;
	if (tx>=Ntheta||ty>=Nrho) return;
	int indg=ty*Ntheta+tx;
	st[indg]+=ss;
	nvals[indg]-=ss;
}
...
__global__ void pmul (float* df,float* dfit,int N1,int N2,int ni)
{
	...
	int tz = blockIdx.z*blockDim.z + threadIdx.z;
	if (tx>=N1||ty>=N2||tz>=ni) return;
	df[tz*N1*N2+ty*N1+tx]=df[tz*N1*N2+ty*N1+tx]*dfit[ty*N1+tx];
}

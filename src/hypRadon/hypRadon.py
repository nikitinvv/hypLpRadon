...
class hypRadon:
    def __init__(self, t, x, q, tau, ni, Ntheta, Nrho):
        ...
        [Ntau, Nq] = shape(q)
        T0 = t[0, 0]
        T1 = t[-1, -1]+t[0, 1]-t[0, 0]
        X1 = x[-1, -1]+x[1, 0]-x[0, 0]
        t = t/T1
        x = x/X1
        q = q*X1/T1
        tau = tau/T1
        ...
        q2 = q**2
        gamma = 0
        alpha = (max(ndarray.flatten(arctan(q2))) +
                 min(ndarray.flatten(arctan(q2))))/2

        beta = max(ndarray.flatten(arctan(q2))) - \
            min(ndarray.flatten(arctan(q2)))
        aR = sqrt(2)*sin(beta)*sqrt(1/(2 + 2*sin(2*alpha)
                                       * (cos(beta) + sin(beta)) + sin(2*beta)))
        Ox = -sqrt(0.2e1) * sin(alpha + pi / 0.4e1) * \
            sin(beta) * aR / (-0.1e1 + cos(beta)) / 0.2e1
        Oy = sqrt(0.2e1) * aR * cos(alpha + pi / 0.4e1) * \
            (0.1e1 - cos(beta)) / sin(beta) / 0.2e1
        ...
        ym1 = -tan(beta/2)*xm1
        am = sqrt(xm1**2+ym1**2)
        # decrease interval size due to the factor T0/T1
        am = am+aR*(T0/T1)**2*0.5
        dtheta = (2*beta)/Ntheta
        drho = -log(am)/Nrho
        thetas = linspace(-beta, beta, Ntheta+1)
        thetas = thetas[:-1]
        rhos = linspace(log(am), 0, Nrho+1)
        rhos = rhos[:-1]
        ...
        tt1 = tt1+Ox
        xx1 = xx1+Oy
        thetat = arctan2(xx1, tt1)
        rhot = log(sqrt(tt1**2+xx1**2))
        emul = exp(reshape(rhot, shape(t)))
        thetat = thetat/2/beta
        rhot = (rhot-log(am)/2)/(-log(am))
        thetat = ndarray.flatten(thetat)
        rhot = ndarray.flatten(rhot)
        #q,tau in lp
        ...
        thetaq = thetaq/2/beta
        rhoq = (rhoq-log(am)/2)/(-log(am))
        thetaq = ndarray.flatten(thetaq)
        rhoq = ndarray.flatten(rhoq)
        # Jacobian hyperbolic integration, divided by 2x
        J = 4*abs((aR**2*t)/(2*(Ox**2 + Oy**2) + aR**2*(1 - 2*t**2 + 2*t**4 - 2*x**2 + 2*x**4) + 2*aR *
                  (Ox*(-1 + 2*t**2) + Oy*(-1 + 2*x**2))*cos(alpha) + 2*aR*(Ox - Oy + 2*Oy*t**2 - 2*Ox*x**2)*sin(alpha)))

        def ker(x): return ((1/6.0*(2-abs(x))**3)*(abs(x) < 2)*(abs(x) >= 1) +
                            (2/3.0-1/2.0*abs(x)**2*(2-abs(x)))*(abs(x) < 1))  # need [-N/2:N/2-1]
        B3theta = ker(theta[0, :]/2/beta*Ntheta)
        B3rho = ker((rho[:, 0]-log(am)/2)/log(am)*Nrho)
        B3thrho = transpose(matrix(B3rho))*matrix(B3theta)
        ...
        fZgpu = ndarray.flatten(
            transpose(array([real(ndarray.flatten(fZgpu)), ndarray.flatten(imag(fZgpu))])))

        # final normalization coefficient
        cc = X1*(t[0, 1]-t[0, 0])*(x[1, 0]-x[0, 0]) / \
            (theta[0, 1]-theta[0, 0])/(rho[1, 0]-rho[0, 0])/aR

        # sorting
        ttn = thetat*Ntheta
        xxn = rhot*Nrho
        add = 2
        idthetatx = int32(floor(ttn))+Ntheta//2+add
        idrhotx = int32(floor(xxn))+Nrho//2+add
        thetae = zeros(size(theta[0, :])+2*add)
        rhoe = zeros(size(rho[:, 0])+2*add)
        thetae[add:-add] = theta[0, :]/2/beta*Ntheta
        rhoe[add:-add] = (rho[:, 0]-log(am)/2)/(-log(am))*Nrho
        ...
        idg = idg[reorids]
        st = zeros((Nrho+2*add)*(Ntheta+2*add))
        nvals = zeros((Nrho+2*add)*(Ntheta+2*add), dtype=int32)
        k = 0
        while(k < Nt*Nx):
            j = 1
            st[idg[k]] = k
            ...
            k = k+j
        st[idg[Nt*Nx-1]+1:] = st[idg[Nt*Nx-1]] + \
            j  # norm(st(idg+1)-st(idg)-nvals(idg))
        # fwd2
        ttn = thetaq*Ntheta
        xxn = rhoq*Nrho
        ...
        stadj = zeros((Nrho+2*add)*(Ntheta+2*add))
        nvalsadj = zeros((Nrho+2*add)*(Ntheta+2*add), dtype=int32)
        k = 0
        while(k < Nq*Ntau):
            j = 1
            stadj[idgadj[k]] = k
        ...
        st = int32(st)
        nvals = int32(nvals)
        reorids = int32(reorids)
        stadj = int32(stadj)
        nvalsadj = int32(nvalsadj)
        reoridsadj = int32(reoridsadj)
        idthetatx = int32(idthetatx)
        idrhotx = int32(idrhotx)
        idthetatauq = int32(idthetatauq)
        idrhotauq = int32(idrhotauq)
        ...
        cc = float32(cc)
        emul = float32(emul)
        thetat = float32(thetat)
        rhot = float32(rhot)
        cosmul = float32(cosmul)
        thetaq = float32(thetaq)
        rhoq = float32(rhoq)
        dthetatx = float32(dthetatx)
        drhotx = float32(drhotx)
        dthetatauq = float32(dthetatauq)
        drhotauq = float32(drhotauq)

        emul = ndarray.flatten(emul)
        cosmul = ndarray.flatten(cosmul)
        J = ndarray.flatten(J)
        fZgpu = float32(fZgpu)

        self.lpRgpu0 = lpRgpu(asarray([Nt, Nx, Nq, Ntau, Ntheta, Nrho, ni]), fZgpu, st, stadj, idthetatx, idrhotx,
                              idthetatauq, idrhotauq, dthetatx, drhotx, dthetatauq, drhotauq, emul, cosmul, J, reorids, reoridsadj)
        self.cc = cc
        self.Nt = Nt
        self.Nx = Nx
        self.Ntau = Ntau
